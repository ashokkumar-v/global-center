<header>

<div class="mob-social-header hidden-md hidden-lg">
<div class="container">
<!--<div class="aff-no col-lg-5 col-md-5 col-sm-5 col-xs-5">
Affiliation No : 123456
</div>

<div class="downloads col-lg-7 col-md-7 col-sm-7 col-xs-7">
<a href="https://gaitview.com/gvt/login.html" target="_blank">Parents Login</a>
</div>-->

<div class="col-md-8 col-sm-8 no-pad-menu">
<div class="conatct-detail">

<!--<div class="downloads">
<a href="downloads.html">Downloads</a>
</div>-->
<div class="phone-num">
+44 74597 13276, +44 74382 17913
</div>
</div>
</div>
<div class="col-md-4 col-sm-4">
<div class="folow-us">

<a href="https://www.facebook.com/globalcenterofarts" target="_blank"><img src="images/facebook.png" alt="social"/></a>
<img src="images/you-tube-icon.png" alt="social"/>
</div>
</div>
</div>
</div>


<div class="social-header">
<div class="container">
<div class="col-md-3 col-sm-3">
<div class="folow-us">
Follow us on :
<a href="https://www.facebook.com/globalcenterofarts" target="_blank"><img src="images/facebook.png" alt="social"/></a>
<img src="images/you-tube-icon.png" alt="social"/>
</div>
</div>
<div class="col-md-9 col-sm-9 no-pad-menu">
<div class="conatct-detail">
<!--<div class="Parents-login">
<a href="https://gaitview.com/gvt/login.html" target="_blank">Parents Login</a>
</div>
<div class="downloads">
<a href="downloads.html">Downloads</a>
</div>-->

<div class="phone-num">
    <i class="fa fa-phone" aria-hidden="true"></i>
+44 74597 13276, +44 74382 17913
</div>
<div class="emailaddress">
    <i class="fa fa-envelope" aria-hidden="true"></i>
<a href="mailto:globalcenterofarts@gmail.com">globalcenterofarts@gmail.com</a>
</div>
<!--<div class="aff-no">
Affiliation No : 12345
</div>-->
</div>
</div>
</div>
</div>

<div class="clearfix"></div>
<div class="menu-part">
<div class="container">
<div class="col-md-3 col-sm-3 col-xs-6 small-dev-logo">
<div class="logo-wisewoods">
<a href="index.html"><img src="img/GCA-logo.png" alt="logo" class="img-responsive" style="height:92px;"/></a>
</div>
</div>
<div class="col-md-9 col-sm-9 col-xs-12 no-pad-menu">
<div class="menu">
<div class="main_menu">
          <nav class="navbar navbar-inverse no-pad-sd menu-smal-dev">
            <div class="container-fluid no-pad-sd">
              <div class="navbar-header">
                <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#myNavbar"> <span class="icon-bar"></span> <span class="icon-bar"></span> <span class="icon-bar"></span> </button>
              </div>
              <div class="collapse navbar-collapse no-pad-sd" id="myNavbar">
                <ul class="nav navbar-nav">
                  <li class="active"><a href="index.php">HOME</a></li>
					        <li class="dropdown dropdown-submenu"><a href="#" class="dropdown-toggle" data-toggle="dropdown">Global Academy of Arts  <b class="caret"></b> </a>
                    <ul class="dropdown-menu">
                      <li><a href="about-us_gca.php">About us</a></li>
                      <li><a href="courses.php">Courses & Exams</a></li>
                      <li><a href="faculty_gca.php">Branches & Teachers</a></li>
                    </ul>
                  </li>
                  <li class="dropdown dropdown-submenu"><a href="#" class="dropdown-toggle" data-toggle="dropdown">Who we are?  <b class="caret"></b></a>
                    <ul class="dropdown-menu">
                      <li><a href="affiliations_gca.php">Our Affiliations</a></li>
                      <li><a href="members_gca.php">Our Members </a></li>
                      <li><a href="committee_gca.php">Executive Committee</a></li>
                    </ul>
                  </li>
					 
					        <li class="dropdown dropdown-submenu"><a href="#" class="dropdown-toggle" data-toggle="dropdown">News & Blogs  <b class="caret"></b></a>
                    <ul class="dropdown-menu">
                      <li><a href="#">Latest News</a></li>
                      <li><a href="#">Blog</a></li>
                      <li><a href="#">Newsletters </a></li>
                    </ul>
                  <li><a href="career.php"> Careers</a></li>
                  <li class="dropdown dropdown-submenu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown"> Global Tamil School <b class="caret"></b></a>
                    <ul class="dropdown-menu">
                      <li><a href="about-us_gts.php">About us</a></li>
                      <li><a href="curriculam.php">Online classes & Syllabus</a></li>
                      <li><a href="faculty_gts.php">Branches & Teachers</a></li>
                      <li><a href="affiliations_gts.php">Our Affiliations</a></li>
                    </ul>
                  </li>
                  <li><a href="contact-us.php"> Contact us</a></li>
                </ul>
              </div>
            </div>
          </nav>
   </div>
</div>
</div>
</div>
</div>
</header>